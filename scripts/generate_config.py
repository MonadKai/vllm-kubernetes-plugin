...
from scripts.utils import find_logger_modules, find_methods_with_request_id
...
def write_python_config_file(config: PackageScannedInfo, output_path: Path) -> None:
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            "# This file is automatically generated by scripts/generate_config.py, do not modify manually\n\n"
        )
        f.write(f"SCANNED_INFO = {repr(config.model_dump())}\n")
...
def main():
    ...
    project_root = Path(__file__).parent.parent
    config_dir = project_root / "src" / "vllm_kubernetes_plugin" / "config"
    ...
    for package_name in package_names:
        ...
        config_file = config_dir / f"{package_name}_scanned_info.py"
        write_python_config_file(config, config_file)

        # Write JSON file for validation (not included in package)
        json_file = test_output_dir / f"{package_name}_scanned_info.json"
